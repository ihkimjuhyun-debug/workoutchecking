...
// DALL-E 이미지가 있으면 렌더링, 없으면 텍스트(첫 2글자) 렌더링
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 🔥 버그 수정 2: 검색 화면 카테고리 필터 단일 선택 이벤트
...
// 🔥 화면 라우팅 버그 수정: 모든 뷰를 확실히 가리고 타겟만 노출
...
// ------------------------------------------------------------------------
// 🌐 하이브리드 API (Local Key vs Vercel Serverless Proxy)
// ------------------------------------------------------------------------
function saveApiKey() {
...
async function callOpenAI(type, bodyData) {
  ...
  try {
    if (localKey) {
      ...
      // Vercel 배포 시 /api 폴더 안의 서버리스 함수 호출
    ...
    throw new Error(error.message || "네트워크 연결 또는 API 서버 오류입니다.");
  }
}

// ------------------------------------------------------------------------
// 🎨 DALL-E 커스텀 썸네일 자동 생성기
// ------------------------------------------------------------------------
async function generateDalleIcon() {
  ...
  btn.innerText = "🎨 AI 통신 중... 잠시만 기다려주세요."; 
  btn.disabled = true;
...
async function generateAIRoutines() {
  ...
  try {
    const data = await callOpenAI('chat', {
      model: "gpt-4o-mini", // Vercel Serverless Timeout 우려 시 mini 사용 권장
    ...
    document.getElementById('ai-routines-list').innerHTML = generatedRoutinesTemp.map((r, i) => `
      <div class="ai-routine-card" id="routine-card-${i}" onclick="selectRoutine(${i})">
        ...
        <div style="margin-top:10px; font-size:0.8rem; color:var(--primary);">총 ${r.sessionsCount}회 세션 예정</div>
      </div>`).join('');
  } catch (err) {
    alert("API 호출 실패: " + err.message + "\n(임시 개발용 데이터로 렌더링합니다.)");
    generatedRoutinesTemp = [{ title: "1. 안전한 볼륨 루틴", desc: "주 2회 안정적인 볼륨으로 적응합니다.", sessionsCount: months * 8 }];
    document.getElementById('ai-loading').style.display = 'none'; 
    document.getElementById('ai-routines-area').style.display = 'block';
    document.getElementById('ai-routines-list').innerHTML = `<div class="ai-routine-card" id="routine-card-0" onclick="selectRoutine(0)"><div class="ai-routine-title">임시 루틴</div><div class="ai-routine-desc">오류로 인한 임시 생성 데이터입니다.</div></div>`;
  }
}
...
function startSelectedRoutine() {
  ...
  for(let i=0; i<routine.sessionsCount; i++) {
    ...
    sessions.push({ id: i, date: `${sDate.getFullYear()}.${String(sDate.getMonth()+1).padStart(2,'0')}.${String(sDate.getDate()).padStart(2,'0')}`, title: `${target.split(' ')[0]} 훈련 (Day ${i+1})`, done: false });
  }
...
function renderActiveGoal(goal) {
  ...
  document.getElementById('goal-sessions-list').innerHTML = goal.sessions.map((s, idx) => s.done ? 
...
function resetGoal() { 
  if(confirm("정말 현재 진행 중인 목표를 삭제하시겠습니까?")) { 
...
function startWorkout() {
  ...
  addExerciseInput(); renderQuickSelectCards(); showView('record-view');
}
  ...
  card.innerHTML = `
    ...
    <div class="ex-card-right">
      <div class="ex-header">
        ...
        <div class="ex-icons">
          <span class="action-icon ${ex.fav?'icon-fav active':'icon-fav'}" onclick="event.stopPropagation(); toggleFavorite('${ex.name}')">⭐</span>
...
function openExerciseHistory(name) {
  ...
  if(!exercises[name].history || exercises[name].history.length === 0) {
    container.innerHTML = '<div style="color:#888;">기록이 없습니다.</div>';
  } else {
...
function renderHallOfFame() {
  const sessions = JSON.parse(localStorage.getItem('pr_sessions')); 
  const volList = document.getElementById('hof-volume-list'); volList.innerHTML = '';
...
function openSessionDetail(key) {
  ...
  currentEditingSessionKey = key; 
  document.getElementById('session-detail-title').innerText = `${session.date.split(' ')[0]} 상세`; 
  document.getElementById('session-detail-info').innerText = `총 볼륨: ${session.totalVolume.toLocaleString()}kg`;
...
function searchExercises() {
  ...
  const filterCat = activeBtn ? activeBtn.dataset.filter : null;
...
function addExerciseFromQuickSelect(exName) { 
  const inputs = document.querySelectorAll('.exercise-entry .input-name'); let filled = false; 
  for (let input of inputs) { 
    if (input.value.trim() === '') { 
      input.value = exName; filled = true; input.closest('.exercise-entry').style.boxShadow = '0 0 15px rgba(255, 140, 0, 0.6)'; setTimeout(() => { input.closest('.exercise-entry').style.boxShadow = 'none'; }, 600); break; 
    } 
  } 
...
function goBackFromSessionDetail() { showView(previousViewBeforeSession); } 
